...
```bash
apimetrics list-results --from <ISO> --time <ISO> -o json   # {meta, results:[...]}, page via meta.next_cursor
```

(`list-results` takes `--from` (start) and `--time` (end), plus `--result-category`/`--cursor`/`--limit`.)
