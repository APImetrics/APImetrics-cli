...
- SLO — `get-project-slo` (bare single object; one SLO per project, scoped via `include_tags`/`exclude_tags`, not a list of named SLOs)
...
- which schedules a call belongs to — `list-schedules-by-call <call-id>`
