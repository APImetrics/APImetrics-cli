...
- SLO — `get-project-slo` (bare single object; there is no `list-slos`/`get-slo` — SLOs are one per project, scoped via `include_tags`/`exclude_tags`, not a list of named SLOs)
...
- recent results — `list-results --from <ISO>` for working-vs-configured coverage
- which schedules a call belongs to — `list-schedules-by-call <call-id>` (not `list-schedules-for-call`)
